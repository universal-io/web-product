...
- **ページ構成（1セクション = 1画面・100dvh）**: スクラブ演出は試作の結果いったん廃止し、
  独立した 2 シーン + フッターの縦積み構成にしている
  1. **シーン1 = ヒーロー**: 左にコピー、右に「入力キャプチャ」演出
  2. **シーン2 = モニタリング（`#monitor`）**: 左に膜パネル（ダッシュボード）、右に
     「あなた自身の生きたダッシュボード」のコピー（`vision.monitor`）。気分・メンタルヘルスの可視化と
     セルフケア訴求。診断ではない旨のディスクレーマー付き。背景はドットグリッドの「データルーム」で
     シーン1と意図的に別世界にしている
  3. **フッター**: シリコンバレー系スタートアップのトーン（`VisionFooter.tsx`）。ステータスドット、
     モノスペースのマイクロコピー、リンク列（すべて仮リンク）、下端で切れる巨大ウォーターマーク
- **主要コンポーネント**:
  - `src/components/vision/VisionHero.tsx` — ナビ・2シーンのレイアウトとコピー・背景
  - `src/components/vision/InputCapture.tsx` — 「入力キャプチャ」。ホットキー（⌥ space）で Mac ネイティブ風の
    入力ウィンドウが立ち上がり、イライラした文章が打ち込まれ、膜がトーンを読み取ってやわらかな言い換えを
    返すループアニメーション（すべて演出。操作不可）
  ...
  - `src/components/vision/VisionFooter.tsx` — vision 専用フッター
...
日々のツール使用（入力）という活動そのものが、そのままユーザーの状態（気分・メンタルヘルス等）の
可視化になっていく、という流れで見せる。トップでは「ホットキー→入力→レビュー→言い換え」の
入力サポート演出が正解。次のシーンで同じシグナルがダッシュボードとして表現し直される。
登場する「入力」や「アプリ画面」は実際に操作できる機能ではなく、あくまで JS アニメーションによる演出。
（補足: スクロールスクラブでの連続変形も一度試したが、挙動が安定せず 2 シーン分割に変更した経緯あり）
